...
SELECT a.id, b.id
FROM (
	SELECT id, '重訓' AS skill_name
	FROM "COACH"
	) AS a
INNER JOIN (
	SELECT id, name AS skill_name
	FROM "SKILL"
	WHERE name = '重訓'
	) AS b ON a.skill_name = b.skill_name;

INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
SELECT a.id, b.id
FROM (
	SELECT "COACH".id AS id, '瑜伽' AS skill_name
	FROM "COACH", "USER"
	WHERE "COACH".user_id = "USER".id AND "USER".email = 'muscle@hexschooltest.io'
	) AS a
INNER JOIN (
	SELECT id, name AS skill_name
	FROM "SKILL"
...
SELECT a.id, b.id
FROM (
	SELECT "COACH".id AS id
	FROM "COACH", "USER"
	WHERE "COACH".user_id = "USER".id AND "USER".email = 'starplatinum@hexschooltest.io'
	) AS a
CROSS JOIN (
	SELECT id
	FROM "SKILL"
	WHERE name IN ('有氧運動', '復健訓練')
	) AS b;
...
FROM "USER"
WHERE "USER"."id" = "COACH"."user_id" AND "USER"."email" = 'starplatinum@hexschooltest.io';
...
FROM "USER"
WHERE "COURSE_BOOKING".user_id = "USER".id AND "USER".name = '王小明';
...
SELECT "USER".name AS 學員, "COURSE_INFO".course_name AS 課程名稱, "COURSE_INFO".user_name AS 授課教練, "COURSE_BOOKING".booking_at AS 預約時間, "COURSE_BOOKING".STATUS AS 課程狀態
FROM "COURSE_BOOKING"
INNER JOIN "USER" ON "COURSE_BOOKING".user_id = "USER".id
INNER JOIN (
	SELECT "COURSE".id, "COURSE".name AS course_name, "USER".name AS user_name
	FROM "COURSE"
	INNER JOIN "USER" ON "COURSE".user_id = "USER".id
	) AS "COURSE_INFO" ON "COURSE_BOOKING".course_id = "COURSE_INFO".id
WHERE "USER".name = '王小明';
...
FROM "USER"
WHERE "COURSE_BOOKING".user_id = "USER".id AND "USER".name = '王小明' AND "status" != '課程已取消';

-- 5-6. 查詢：計算用戶王小明的購買堂數，顯示須包含以下欄位： user_id , total。 (需使用到 SUM 函式與 Group By)
SELECT "USER".id AS user_id, SUM("CREDIT_PACKAGE".credit_amount) AS total
FROM "USER"
...
SELECT "USER".id AS user_id, COUNT(*) AS total
FROM "USER"
INNER JOIN "COURSE_BOOKING" ON "USER".id = "COURSE_BOOKING".user_id
WHERE "USER".name = '王小明' AND "COURSE_BOOKING"."status" IN ('上課中', '完成課程')
GROUP BY "USER".id;
...
SELECT "CREDIT_PURCHASE".user_id AS user_id, ("CREDIT_PURCHASE".total - "COURSE_BOOKING".total) AS remaining_credit
FROM (
	SELECT "USER".id AS user_id, SUM("CREDIT_PACKAGE".credit_amount) AS total
	FROM "USER"
	INNER JOIN "CREDIT_PURCHASE" ON "USER".id = "CREDIT_PURCHASE".user_id
...
SELECT COUNT(*) AS coach_total, "SKILL".name AS 專長名稱
...
INNER JOIN "CREDIT_PACKAGE" ON "CREDIT_PURCHASE".credit_package_id = "CREDIT_PACKAGE".id
WHERE "CREDIT_PURCHASE".purchase_at BETWEEN '20241201' AND '20241231'
GROUP BY "CREDIT_PACKAGE"."name";

-- 6-4. 查詢：計算 11 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收
SELECT SUM("CREDIT_PACKAGE"."price") AS 總營收
FROM "CREDIT_PURCHASE"
INNER JOIN "CREDIT_PACKAGE" ON "CREDIT_PURCHASE".credit_package_id = "CREDIT_PACKAGE".id
WHERE "CREDIT_PURCHASE".purchase_at BETWEEN '20241201' AND '20241231';
...
-- 顯示須包含以下欄位： 預約會員人數
SELECT count(*) AS 預約課程會員人數
FROM (
	SELECT DISTINCT user_id
	FROM "COURSE_BOOKING"
	WHERE created_at BETWEEN '20241201' AND '20241231' AND NOT "status" = '課程已取消'
	) AS t;
